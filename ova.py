...
IS_WIN11 = IS_WINDOWS and WIN_RELEASE == "11"
IS_WIN10 = IS_WINDOWS and WIN_RELEASE == "10"

console = Console(
    force_terminal=True,
    color_system="truecolor" if IS_WIN11 else None
)
# ======================================
...
DEFAULT_CONFIG = {
    "gameId": 2753915549,
    "private_link": "",
    "Executor WorkSpace Folder": "C:\\Users\\bang ova\\AppData\\Local\\Potassium\\workspace",
    "check executor": 3,
    "check in game": 60,
    "launchDelay": 15,
...
def check_json_status(workspace_folder, username, launch_time=None, check_executor_minutes=3, pid=None):
    if pid is not None and not is_roblox_process_running(pid):
        return "offline"
    ...
    if launch_time is not None:
        ...
        check_executor_seconds = check_executor_minutes * 60
        
        if time_since_launch < check_executor_seconds:
            if time_diff is None:
                return "waiting_first_check"
            elif time_diff <= 60:
                return "in_game"
            else:
                return "waiting_first_check"
...
# ----------------------------
def match_existing_processes_to_accounts(accounts, workspace_folder, check_executor_minutes):
...
# Main Process Cycle
# ----------------------------
def proc_cycle(accounts, cfg, follow_state):
    ...
    if GAME_CHANGE_STATE["is_changing"]:
        ...
        # Setelah semua dil launch, return untuk skip proses normal
        return
    ...
    launch_delay = float(cfg.get("launchDelay", 15))
    workspace_folder = cfg.get("Executor WorkSpace Folder", "")
    check_executor_minutes = int(cfg.get("check executor", 3))
    ...
    for acc in accounts:
        ...
        status = check_json_status(workspace_folder, name, 
                                   launch_time=launch_time,
                                   check_executor_minutes=check_executor_minutes,pid=pid)

        if status == "offline" and pid and is_roblox_process_running(pid):
            aggressive_kill_process(pid, acc)
            continue
        if pid is not None:
            if not is_roblox_process_running(pid):
                add_log(f"⚠️ Process died: {name}", important=False)
                acc["pid"] = None
                acc["launch_time"] = None
                pid = None
                status = "offline"
            else:
                # Jika proses masih hidup → pakai status JSON normal
                status = check_json_status(
                    workspace_folder, name,
                    launch_time=None,
                    check_executor_minutes=check_executor_minutes,
                    pid=pid
                )
        else:
            # Jika pid None → langsung offline (skip JSON)
            status = "offline"

        need_follow_relaunch = acc.get("follow_relaunch", False)
        
        # FIX: Prevent launch loop - check if already launching/waiting
        is_launching = status in ["waiting_first_check", "launching"]
        recently_launched = (launch_time and (time.time() - launch_time) < (check_executor_minutes * 60))
        
        if pid is None and status == "offline" and not need_follow_relaunch and not is_launching and not recently_launched:
            now = time.time()
            ...
            if now - acc.get("last_launch", 0) >= cooldown:
                ...
                if new_pid:
                    ...
                    add_log(f"❌ Launch failed: {name}", important=True)
        
        elif pid:
            # FIX: Only relaunch if truly offline (after grace period)
            check_executor_seconds = check_executor_minutes * 60
            grace_period_passed = (launch_time is None or 
                                  (time.time() - launch_time) > check_executor_seconds)
            
            should_relaunch = (
                (status == "offline" and grace_period_passed) or 
                need_follow_relaunch
            )
            
            if should_relaunch:
                add_log(f"🔄 Killing {name} before relaunch", important=True)
                aggressive_kill_process(pid, acc)
                ...
                if now - acc.get("last_launch", 0) >= cooldown:
                    ...
                    add_log(f"🔄 Relaunch: {name}", important=True)
...
# ----------------------------
def main():
    global SCRIPT_START_TIME, GAME_CHANGE_STATE, ACCOUNT_CHANGE_STATE, ACCOUNT_ROTATION_INDEX
    ...
    if not workspace_folder or not os.path.exists(workspace_folder):
        console.print(f"WARNING: Workspace folder not found: {workspace_folder}")
        console.print("Please set 'Executor WorkSpace Folder' in config.json")
        return
    ...
    matched = match_existing_processes_to_accounts(
        accounts, 
        workspace_folder, 
        int(cfg.get("check executor", 3))
    )
    if matched > 0:
        console.print(f"Matched {matched} running processes")
    else:
        add_log("No running processes, launching...", important=True)
        proc_cycle(accounts, cfg, follow_state)        

    time.sleep(1)
    console.clear()

    check_in_game_seconds = int(cfg.get("check in game", 60))
    
    live = None
    next_check = time.time()

    # Render awal
    table = Table(title="Loading...", box=None)
    live = start_safe_render(table)
    ...
    while True:
        ...
        if now >= next_check:
            proc_cycle(accounts, cfg, follow_state)
            next_check = now + check_in_game_seconds

            follow_mode = "ENABLED" if cfg.get("FollowPlayer", False) else "DISABLED"
            follow_target = cfg.get("FollowPlayerUsername", "N/A")
            check_interval = cfg.get("check in game", 60)
            check_executor_min = cfg.get("check executor", 3)
            
            title_parts = [f"Roblox Multi-Instance v3"]
            title_parts.append(f"Check: {check_interval}s | First: {check_executor_min}min")
            ...
            for i, acc in enumerate(accounts, start=1):
                ...
                # STATUS FIX: Improve status logic
                if pid is None or not is_roblox_process_running(pid):
                    # No PID or dead process
                    status = "offline"
                elif launch_time is not None:
                    # Process running + has launch_time → check JSON with grace period
                    time_since_launch = time.time() - launch_time
                    grace_period = check_executor_minutes * 60
                    
                    if time_since_launch < grace_period:
                        # Still in grace period → show as waiting
                        status = "waiting_first_check"
                    else:
                        # Grace period passed → check JSON
                        status = check_json_status(
                            workspace_folder, name,
                            launch_time=launch_time,
                            check_executor_minutes=check_executor_minutes,
                    ...
                    # Process running but no launch_time → check JSON without grace
                    status = check_json_status(
                        workspace_folder, name,
                        launch_time=None,
                        check_executor_minutes=check_executor_minutes,
                ...
                time_since_launch_str = ""
                if launch_time is not None:
                    time_since_launch = int(time.time() - launch_time)
                    mins = time_since_launch // 60
                    secs = time_since_launch % 60
                    time_since_launch_str = f" | {mins}m{secs}s"

                if status == "in_game":
                    if time_diff is not None:
                        status_msg = f"In Game ✅ | JSON: {int(time_diff)}s{time_since_launch_str}"
                    else:
                        status_msg = f"In Game ✅{time_since_launch_str}"
                    status_style = "bold green"
                elif status == "offline":
                    if pid:
                        if time_diff is not None:
                            status_msg = f"Offline (Running) | JSON: {int(time_diff)}s{time_since_launch_str}"
                        else:
                            status_msg = f"Offline (Running){time_since_launch_str}"
                        status_style = "bold red"
                    else:
                        if time_diff is not None:
                            status_msg = f"Offline | JSON: {int(time_diff)}s"
                        else:
                            status_msg = "Offline | No JSON"
                        status_style = "bold red"
                elif status == "waiting_first_check":
                    check_executor_seconds = check_executor_minutes * 60
                    if launch_time is not None:
                        remaining = max(0, check_executor_seconds - int(time.time() - launch_time))
                        status_msg = f"⏳ Waiting ({remaining}s){time_since_launch_str}"
                    else:
                        status_msg = "⏳ Waiting First Check"
                    status_style = "bold yellow"
                    
                # BF Stats Safe Loader
                bf = load_bf_stats(cfg.get("Executor WorkSpace Folder", ""), acc["username"])
